...
std::vector<std::string> get_classes(component_vector components){
    auto compare_by_cls = [](chemical_component a, chemical_component b) {return a.cls < b.cls;};
...
component_vector components_per_class(component_vector components, std::string cls){
    component_vector output;
    for(auto i : components) if(i.cls == cls) output.push_back(i);
    // auto compare_by_dbe = [](chemical_component a, chemical_component b) {return a.DBE < b.DBE;};
    // std::sort(output.begin(), output.end(), compare_by_dbe);
    return output;
}

heteroatomic_class::heteroatomic_class(component_vector components, std::string name){
    ...
    if(class_components.size() > 0){
        ...
        class_dbes[dbe_ant] = new DBE(class_components, dbe_ant);
        for(auto i : class_components){
            if(i.DBE != dbe_ant){
                class_dbes[i.DBE] = new DBE(class_components, i.DBE);
            }
...
DBE::DBE(component_vector components, int dbe){
    ...
    auto compare_by_c = [](chemical_component a, chemical_component b) {return a.C < b.C;};
...
sample::sample(component_vector components){
    sample_components = components;
    class_names = get_classes(components);
    for(auto i : class_names){
        classes[i] = new heteroatomic_class(components, i);
    }
    ...
    if(components.size() > 0){
        ...
        dbes[dbe_ant] = new DBE(components, dbe_ant);
        for(auto i : components){
            if(i.DBE != dbe_ant){
                dbes[i.DBE] = new DBE(components, i.DBE);
            }
            dbe_ant = i.DBE;
            intensity += i.intensity;
        }
...
float heteroatomic_class::sum_dbe(int min, int max){
    float sum = 0.0;
    for(int i = min; i <= max; i++) sum += get_DBE(i)->intensity;
    return sum;
}

float heteroatomic_class::sum_dbe(std::vector<int> vals){
    float sum = 0.0;
    for(auto i : vals) sum += get_DBE(i)->intensity;
    return sum;
}

float DBE::sum_c(int min, int max, int opt){
    float sum = 0.0;
...
float DBE::sum_c(std::vector<int> vals){
    float sum = 0.0;
    for(auto i : vals) sum += c_intensity[i].x;
    return sum;
}

heteroatomic_class *sample::get_class(std::string name){
    try {
        return classes.at(name);
    }
    catch (const std::out_of_range& oor) {
        return new heteroatomic_class(sample_components, name);
    }
}

DBE *sample::get_DBE(int num){
    try {
        return dbes.at(num);
    }
    catch (const std::out_of_range& oor) {
        return new DBE(sample_components, num);
    }
}

DBE *heteroatomic_class::get_DBE(int num){
    try {
        return class_dbes.at(num);
    }
    catch (const std::out_of_range& oor) {
        return new DBE(class_components, num);
    }
}

float heteroatomic_class::C(int dbe, int c){
    return get_DBE(dbe)->c_intensity[c].x;
}

float heteroatomic_class::sum_C(int dbe, int min, int max, int opt){
    return get_DBE(dbe)->sum_c(min, max, opt);
}

float heteroatomic_class::sum_C(int dbe, std::vector<int> vals){
    return get_DBE(dbe)->sum_c(vals);
}

void sample::print_relative_abundancy(std::ostream &output){
...
void sample::print_biodegradation(std::ostream &output){
    output << std::fixed << std::setprecision(6);
...
void sample::print_paleoenvironment(std::ostream &output){
    auto O2 = *get_class("O2");
    auto O1 = *get_class("O");
    ...
    output << "Par/Impar (FA): " << (O2.sum_C(1, 20, 36, DBE::even))/(O2.sum_C(1, 19, 35, DBE::odd)) << "\n";
...
void sample::print_maturity(std::ostream &output){
    auto N = *get_class("N");
...
void heteroatomic_class::print_intensity_per_dbe(std::ostream &output){
...
void DBE::print_intensity_per_c(std::ostream &output){
...
output::output(std::string name, std::string folder){
    study_name = name;
    folder_path = folder;
}

std::ofstream output::new_output(std::string name){
